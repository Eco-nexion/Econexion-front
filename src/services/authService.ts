...
export const authService = {
  ...
  getUserByEmail: async (email: string): Promise<GoogleRegisterResponse> => {
    ...
    const response = await apiClient.get<GoogleRegisterResponse>(
      `/lab/users/exists/${encodeURIComponent(email)}`
    );
  ...
  register: async (data: RegisterForm): Promise<GoogleRegisterResponse> => {
    console.log("📝 Registrando usuario normal...");
    const response = await apiClient.post<GoogleRegisterResponse>(
      "/api/auth/register",
      {
        ...
        nit: data.nit || "",
        rol: data.role, // Backend expects 'rol'
      }
    );
    console.log("✅ Registro normal exitoso:", response.data);
    return response.data;
  },
